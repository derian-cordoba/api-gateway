...
export function ValidationEditor({ value, onChange }: { value?: Config; onChange: (value?: Config) => void }) {
  ...
  >
    <div className="form-grid">
      <FormField label="Required body fields" hint="Top-level JSON fields, one per line.">
        <StringListInput value={config.requiredFields} onChange={(requiredFields) => update({ requiredFields })} placeholder={"customerId\namount"} />
      </FormField>
      <FormField label="Allowed content types" hint="Case-insensitive content-type matches.">
        <StringListInput value={config.allowedContentTypes} onChange={(allowedContentTypes) => update({ allowedContentTypes })} placeholder={"application/json\napplication/x-www-form-urlencoded"} />
      </FormField>
      <FormField label="Maximum body size" hint="Bytes. Requests with a larger Content-Length return 413.">
        <NumberInput min={1} value={config.maxBodyBytes} onValue={(maxBodyBytes) => update({ maxBodyBytes })} placeholder="1048576" />
      </FormField>
    </div>
  </FeatureSection>;
}
