...
export function CircuitBreakerEditor({ value, onChange }: { value?: Config; onChange: (value?: Config) => void }) {
  const config = value ?? { threshold: 5, timeout: 30_000 };
  const update = (patch: Partial<Config>) => onChange({ ...config, ...patch });
  return <FeatureSection id="circuitBreaker" title="Circuit breaker" description="Stop sending traffic to an unhealthy upstream." enabled={value !== undefined} onEnabledChange={(enabled) => onChange(enabled ? config : undefined)}>
    <div className="form-grid">
      <FormField label="Failure threshold"><NumberInput min={1} value={config.threshold} onValue={(threshold) => update({ threshold: threshold ?? 1 })} /></FormField>
      <FormField label="Open duration" hint="Milliseconds"><NumberInput min={1} value={config.timeout} onValue={(timeout) => update({ timeout: timeout ?? 1 })} /></FormField>
      <FormField label="Recovery successes"><NumberInput min={1} value={config.successThreshold} onValue={(successThreshold) => update({ successThreshold })} placeholder="1" /></FormField>
    </div>
    <div className="nested-panel">
      <Toggle checked={config.healthCheck !== undefined} onChange={(enabled) => update({ healthCheck: enabled ? { url: "http://localhost:4000/health", intervalMs: 10_000 } : undefined })} label="Use active health checks" />
      {config.healthCheck ? <div className="form-grid form-grid--top-gap">
        <FormField label="Health URL" wide><TextInput type="url" value={config.healthCheck.url} onChange={(event) => update({ healthCheck: { ...config.healthCheck!, url: event.target.value } })} /></FormField>
        <FormField label="Probe interval" hint="Milliseconds"><NumberInput min={1} value={config.healthCheck.intervalMs} onValue={(intervalMs) => update({ healthCheck: { ...config.healthCheck!, intervalMs: intervalMs ?? 1 } })} /></FormField>
