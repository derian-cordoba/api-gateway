    ...
    >
      <div className="segmented-control" aria-label="Upstream mode">
        <button
          ...
          onClick={() => onChange({ ...value, targets: undefined, strategy: undefined, stickyKey: undefined, target: value.target ?? "http://localhost:4000" })}
        >
          Single target
        </button>
        <button
          type="button"
          className={loadBalanced ? "active" : ""}
          onClick={() => onChange({ ...value, target: undefined, targets: value.targets ?? [{ url: "http://localhost:4001" }, { url: "http://localhost:4002" }], strategy: value.strategy ?? "round-robin" })}
        >
          Load balanced
        </button>
      </div>
      ...
      {!loadBalanced ? (
        <div className="form-grid">
          <FormField label="Target URL" hint="The upstream service base URL." wide>
            <TextInput type="url" value={value.target ?? ""} onChange={(event) => update({ target: event.target.value })} placeholder="https://service.internal" />
          </FormField>
        </div>
      ) : (
        <div className="stack">
          <FormField label="Balancing strategy">
            <SelectInput value={value.strategy ?? "round-robin"} onChange={(event) => update({ strategy: event.target.value as ProxyConfig["strategy"], stickyKey: event.target.value === "sticky" ? value.stickyKey ?? "ip" : undefined })}>
          ...
          {value.strategy === "sticky" ? (
            <FormField label="Sticky key" hint='Use ip or a source such as "cookie:session_id".'>
              <TextInput value={value.stickyKey ?? "ip"} onChange={(event) => update({ stickyKey: event.target.value })} />
            </FormField>
          ) : null}
          <div className="collection-editor">
            {(value.targets ?? []).map((target, index) => (
              <div className="collection-row collection-row--target" key={index}>
                <TextInput aria-label={`Target ${index + 1} URL`} type="url" value={target.url} onChange={(event) => {
                  const targets = [...(value.targets ?? [])];
                  targets[index] = { ...target, url: event.target.value };
                  update({ targets });
                }} />
                <NumberInput aria-label={`Target ${index + 1} weight`} min={1} value={target.weight} onValue={(weight) => {
                  const targets = [...(value.targets ?? [])];
                  targets[index] = { ...target, weight };
                  update({ targets });
                }} placeholder="Weight" />
                <button type="button" className="icon-button" aria-label={`Remove target ${index + 1}`} disabled={(value.targets?.length ?? 0) <= 2} onClick={() => update({ targets: value.targets?.filter((_, targetIndex) => targetIndex !== index) })}>
                  <Trash2 size={16} />
                </button>
              </div>
            ))}
            <button type="button" className="button button--quiet button--small" onClick={() => update({ targets: [...(value.targets ?? []), { url: "http://localhost:4000" }] })}>
      ...
      <div className="form-grid form-grid--top-gap">
        <FormField label="HTTP method">
          <SelectInput value={value.method ?? ""} onChange={(event) => update({ method: (event.target.value || undefined) as ProxyConfig["method"] })}>
        ...
        <FormField label="Timeout" hint="Milliseconds before the upstream request is cancelled.">
          <NumberInput min={1} value={value.timeout} onValue={(timeout) => update({ timeout })} placeholder="30000" />
        </FormField>
      </div>
      ...
      <div className="subsection-grid">
        <div>
          ...
          <KeyValueEditor value={value.pathRewrite} onChange={(pathRewrite) => update({ pathRewrite })} keyPlaceholder="^/api" valuePlaceholder="" />
        </div>
        <div>
          <h3>Upstream headers</h3>
          <p>Headers added to every proxied request.</p>
          <KeyValueEditor value={value.headers} onChange={(headers) => update({ headers })} keyPlaceholder="X-Service-Key" valuePlaceholder="Value" />
        </div>
      </div>
    </FeatureSection>
  );
