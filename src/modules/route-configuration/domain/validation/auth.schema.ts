...
const AuthRateLimitSchema = z.object({
  max: z
    .number()
    .int()
    .positive("authRateLimit.max must be a positive integer"),
  windowMs: z
    .number()
    .int()
    .positive(
      "authRateLimit.windowMs must be a positive number (milliseconds)",
    ),
});
...
const JwtAuthSchema = z
  ...
  .refine(
    ...
    {
      message:
        "jwt auth requires either secret (HMAC), publicKey (RSA/EC), or jwksUri (JWKS endpoint)",
      path: ["secret"],
    },
  );
...
const OAuth2AuthSchema = z.object({
  ...
  introspectionCacheTtlMs: z
    ...
    .int()
    .positive(
      "introspectionCacheTtlMs must be a positive integer (milliseconds)",
    )
    .optional(),
  authRateLimit: AuthRateLimitSchema.optional(),
});
